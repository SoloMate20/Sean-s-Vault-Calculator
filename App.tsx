      ...
      <main className="container mx-auto p-4 md:p-8">
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              <Card>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <button type="submit" disabled={isCalculating || solPriceLoading} className="w-full flex justify-center items-center bg-brand-yellow hover:bg-brand-yellow/90 text-brand-bg-dark font-bold py-3 px-4 rounded-md transition duration-300 disabled:bg-brand-text-muted/50 disabled:cursor-not-allowed">
              ...
              {result && (
                <div className="space-y-8">
                  <Card>
                    <h2 className="text-2xl font-bold text-gray-50 mb-4">Results Summary</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-center">
                      ...
                      <div className="flex flex-col justify-center items-center p-4 bg-brand-bg-dark/50 rounded-lg">
                        ...
                        <p className="text-sm text-brand-text-muted">{formatCurrency(convertSolToUsd(result.finalBalance), 'USD')} / {formatCurrency(convertUsdToGbp(convertSolToUsd(result.finalBalance)), 'GBP')}</p>
                      </div>
                      <div className="flex flex-col justify-center items-center p-4 bg-brand-bg-dark/50 rounded-lg">
                        ...
                        <p className="text-sm text-brand-text-muted">{formatCurrency(convertSolToUsd(result.totalInterest), 'USD')} / {formatCurrency(convertUsdToGbp(convertSolToUsd(result.totalInterest)), 'GBP')}</p>
                      </div>
                      <div className="flex flex-col justify-center items-center p-4 bg-brand-bg-dark/50 rounded-lg">
                        ...
                        <p className="text-sm text-brand-text-muted">{formatCurrency(convertSolToUsd(result.totalFees), 'USD')} / {formatCurrency(convertUsdToGbp(convertSolToUsd(result.totalFees)), 'GBP')}</p>
                      </div>
                    </div>
                  </Card>
                  <Card>
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                      <LineChart data={result.monthlyData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                        ...
                        <YAxis stroke="#bbc9cc" tickFormatter={(tick) => tick.toFixed(2)} />
                        <Tooltip contentStyle={{ backgroundColor: '#013a33', border: '1px solid #bbc9cc50', borderRadius: '0.5rem' }} />
                        <Legend />
                        <Line type="monotone" dataKey="endBalance" name={`Balance (${input.currency === 'USD' && solPrice ? 'USD' : 'SOL'})`} stroke="#f0c555" activeDot={{ r: 8 }} strokeWidth={2} />
                      </LineChart>
                    </ResponsiveContainer>
                  </Card>
                  <Card>
            ...
            {/* Sidebar */}
            <div className="space-y-8">
              {gbpRate && !gbpRateLoading && !gbpRateError && (
                <CurrencyConverter rate={gbpRate} baseCurrency="USD" targetCurrency="GBP" />
              )}
            </div>
          </div>
        </div>
